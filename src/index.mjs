...
async function annotatePackage(packageName, filePath, lineNumber, ecosystem) {
  try {
    ...
    if (response && response.source) {
      ...
      logFunction(
        `Package ${packageName} (${ecosystem}): (Maturity: ${maturityValue}, Health: ${healthRiskValue}). ${recommendation}`,
        {
          file: filePath,
    ...
    core.error(
      `Error looking up package ${packageName} (${ecosystem}): ${error.message}`,
      {
        file: filePath,
        startLine: lineNumber,
        endLine: lineNumber,
  ...
  try {
    ...
    for (const lineNumber of modifiedLineNumbers) {
      ...
      if (typeof line !== "string") {
        core.warning(
          `Skipping non-string line at line number ${lineNumber}: ${JSON.stringify(line)}`,
        );
        continue;
      }
...
async function run() {
  ...
  const annotateModifiedOnly =
    core.getInput("annotate-modified-only") === "true";
